;(function () {
    ...
    // RSVP Pop-up Functions
    const events = ['Mehendi', 'Haldi', 'Sangeet', 'Wedding', 'Vratam'];
    let currentEventIndex = 0;
    let responses = {};

    function openRSVPPopups() {
        alert("RSVP Pop-ups opening..."); // Confirm JavaScript execution
        currentEventIndex = 0;
        responses = {};
        showPopup(events[currentEventIndex]);
    }
    ...
    function showPopup(event) {
        ...
        popupQuestion.textContent = `Are you attending ${event}?`;

        // Apply specific styles based on the event
        popupContent.className = `popup-content ${event.toLowerCase()}`;
    ...
    function submitResponse(answer) {
        responses[events[currentEventIndex]] = answer;
        currentEventIndex++;
        closePopup();
    ...
    function submitForm() {
        ...
        // Append responses to form as hidden inputs
        for (const [event, response] of Object.entries(responses)) {
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = event.toLowerCase();
